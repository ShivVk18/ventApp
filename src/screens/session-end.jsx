...
export default function SessionEnd() {
  ...
  const { sessionTime = "0", plan = "basic", autoEnded = "false" } = route.params || {}
  ...
  const getEndMessage = () => {
    if (isAutoEnded) {
      return {
        title: "Time's Up!",
        subtitle: "Your 20-minute session has ended automatically.\nThanks for sharing.",
        emoji: "⏰",
      }
    }
    return {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.mainContent}>
          ...
          <View style={styles.sessionStats}>
            <Text style={styles.sessionDuration}>Session Duration: {formatTime(sessionTimeNum)}</Text>
            <Text style={styles.sessionPlan}>Plan: {plan}</Text>
            {isAutoEnded && <Text style={styles.autoEndedText}>Session ended automatically after 20 minutes</Text>}
          </View>
        </View>

        <View style={styles.buttonContainer}>
          <TouchableOpacity style={styles.newVentButton} onPress={handleNewVent}>
